...
import { FileText, Download, Shield, BarChart3, Zap, CheckCircle2, Clock } from 'lucide-react';
  ...
  const downloadJson = () => {
    const reportData = {
      metadata: {
        ...
        workflow_id: `AEGIS-${Math.random().toString(36).substr(2, 9).toUpperCase()}`
      },
    ...
    a.href = url;
    a.download = `AEGIS_REPORT_${strategyName.replace(/\s+/g, '_')}.json`;
    a.click();
  };
    ...
    <div className="bg-[#1e1e1e] rounded-2xl border border-[#333] overflow-hidden shadow-2xl print:bg-white print:text-black print:border-0">
      ...
      <div className="px-8 py-4 bg-[#252526] border-b border-[#333] flex justify-between items-center print:hidden">
        <div className="flex items-center space-x-2 text-[#858585]">
          ...
          <span className="text-[11px] font-bold uppercase tracking-widest">Operational Intelligence Report</span>
        </div>
        <div className="flex items-center space-x-3">
          ...
          <button 
            onClick={printPdf}
            className="flex items-center space-x-2 px-3 py-1.5 rounded bg-indigo-600 hover:bg-indigo-500 text-[11px] font-bold text-white transition-colors"
          >
      ...
      <div className="p-12 space-y-12 max-w-5xl mx-auto print:p-0 print:max-w-none">
        ...
        <div className="flex justify-between items-start border-b-2 border-[#333] pb-8 print:border-black">
          ...
          <div className="text-right">
             ...
             <div className="text-xl font-bold text-white print:text-black">AEGIS ORCHESTRATOR v1.2</div>
          </div>
        </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-12">
            <div className={`p-8 rounded-xl border-2 bg-opacity-5 ${
              context.deployment_decision === 'DEPLOY' ? 'bg-emerald-500 border-emerald-500/30' : 'bg-red-500 border-red-500/30'
            }`}>
              <div className="text-[11px] font-bold uppercase tracking-widest mb-2 opacity-60">Recommendation</div>
              <div className={`text-4xl font-black mb-4 ${
                context.deployment_decision === 'DEPLOY' ? 'text-emerald-500' : 'text-red-500'
              }`}>{context.deployment_decision}</div>
              <p className="text-[14px] leading-relaxed text-[#cccccc] print:text-gray-700 font-medium italic">
        ...
        <div className="border-t border-[#333] pt-8 flex justify-between items-center text-[10px] text-[#555] font-mono uppercase tracking-widest print:border-black print:text-gray-500">
           <div>Aegis Evaluation Pipeline • Room 3 Final Output</div>
